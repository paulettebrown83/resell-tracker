...
export async function requestResaleOperation(input: OperationRequest, requestId: string): Promise<string> {
  ...
  await requireAccess()
  const { data, error } = await supabase.rpc(input.requested.scope === 'poshmark_private_listing_reference' ? 'resale_request_listing_reference' : 'resale_request_operation', { p_request_id: requestId, p_payload: input })
  if (error) throw error
