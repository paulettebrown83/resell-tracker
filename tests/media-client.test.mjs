...
function setup(options={}) {
 const calls=[], urls=[], revoked=[];let attempts=0;
 const session={access_token:'test-token'};
 const supabase={auth:{async getSession(){return {data:{session:options.signedOut?null:session}}}},async rpc(name,args){calls.push({name,args});
 ...
 const context={exports:{},require(name){assert.equal(name,'./supabase');return {supabase,requireWritableDeployment(){if(options.preview)throw Error('preview read only')}}},
  ...
  async fetch(url,init){urls.push({url,init});assert.equal(init.headers.Authorization,'Bearer test-token');assert.equal(init.credentials,'omit');
 ...
 vm.runInNewContext(compiled,context);return {api:context.exports,calls,urls,revoked};
}
