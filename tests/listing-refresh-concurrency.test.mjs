...
try {
  ...
  await admin.query("insert into public.resale_listings(id,account_id,external_listing_id) values($1,$2,'1234567890abcdef12345678')",[listing,account]);
  const payload={account_id:account,action:'import',listing_id:listing,inventory_id:null,expected_observation_id:null,expected_item_version:null,trigger:{kind:'member_request',id:request},requested:{scope:'exact_listing_refresh',external_listing_id:'1234567890abcdef12345678'}};
  for(const c of [a,b])await c.query("select set_config('request.jwt.claim.sub',$1,false)",[owner]);
  const create=c=>c.query('select public.resale_request_listing_refresh($1,$2) id',[request,payload]);
  ...
  await a.query('begin');await a.query('select public.resale_claim_listing_refresh($1)',[op]);const second=b.query('select public.resale_claim_listing_refresh($1)',[op]).then(()=>null,e=>e.code);await waitForLock();await a.query('commit');assert.equal(await second,'55000');
