...
### Pagination rollout checkpoint (2026-09-06)

Reviewed migration `20260906091712_resale_ebay_listing_runs.sql` was applied under hosted ledger `20260906100753`, SHA256 `652195ba3b24267e6e71b8e85124a9417a09451342360984a6575c6f274f788c`. All44 selected pre-existing table fingerprints, including the newly released private marketplace-photo records, remained unchanged. The scoped prerequisite snapshot restored7 captured rows across4 tables plus the captured source-immutability definition; it is not full project recovery.

Nine hosted SQL-role checks passed with all synthetic writes rolled back. Edge `resale-ebay-connect` version2 is deployed; the new run operations reject signed-out requests401 and invalid bearer403. Configuration, connections and retained listing runs remain absent. These are hosted infrastructure checks, not real eBay API acceptance. The app integration supplies automatic page progression and Resume; production browser verification is recorded separately after release.
