...
**The redesigned workbench, private photo service, historical marketplace staging, manual item matching and draft database are deployed. Guided draft preparation and the hosted-tested operations database are deployed. This release adds the shared shop-activity interface, contextual requests and evidence proposals; native visual QA of the production activity screens passed in Comet. The Gmail notification reader has completed real owner consent and an initial bounded production read. Shop Activity shows recognized item/notice/received-time context and clear quarantine reasons; source evidence does not record a sale or remove a listing. Scheduled coverage has its own acceptance gate, and this does not establish continuous marketplace synchronization. See [Gmail connection and background intake](docs/GMAIL_FEED.md).** Read [the rollout and recovery runbook](docs/ROLLOUT.md) and [the resale data contract](docs/RESALE_DATA.md) before changing production. The protected genealogy interface is available at `/genealogy`. See [draft preparation](docs/RESALE_DRAFTS.md) and [the shared operation contract](docs/RESALE_OPERATIONS.md) for exact implementation boundaries. A queued request or checked source record is not proof that a shop listing changed.

Marketplace listing cards also show observed automatic price settings and floors. These observations never change shop prices or confirm physical stock; remote price execution remains blocked until a settings-preserving adapter is implemented. See [marketplace pricing](docs/RESALE_PRICING.md).
