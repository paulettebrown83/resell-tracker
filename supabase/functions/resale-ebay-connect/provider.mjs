...
export function orderPage(raw,page,seller){const root=xmlResponse(raw,'GetOrders'),orders=many(root.OrderArray?.Order),total=integer(root.PaginationResult?.TotalNumberOfPages),count=integer(root.PaginationResult?.TotalNumberOfEntries);
 ...
 // BuyerUserID can be a mutable username or an immutable ID depending on developer mode.
 // Until that mode is verified, only the explicitly typed immutable EIAS field binds deletion.
 const deletion_subjects=orders.map(o=>{if(!text(o.EIASToken,512))throw new EbayError('buyer_deletion_binding_unavailable',409);return {eias_sha256:hash(o.EIASToken)};});
